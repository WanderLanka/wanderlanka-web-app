...
const ReportComplain = () => {
  const [formData, setFormData] = useState({
    ...
    category: 'transport',
    complaint: '',
    submittedDate: new Date().toISOString().slice(0, 16), // yyyy-MM-ddTHH:mm
    status: 'new',
    priority: 'medium',
    serviceProvider: '',
    bookingReference: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post('/transport/complaints', payload);
      ...
      if (normalized.id) localStorage.setItem('transLastComplaintId', normalized.id);
  ...
  useEffect(() => {
    const lastId = localStorage.getItem('transLastComplaintId');
    if (lastId && !submittedComplaint) {
      const fetchExisting = async () => {
        try {
          setLoadingResponse(true);
          const res = await api.get(`/transport/complaints/${lastId}`);
          const doc = res?.data?.data || res?.data || {};
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await api.get(`/transport/complaints/${submittedComplaint.id}`);
        const doc = res?.data?.data || res?.data || {};
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <p className="text-slate-600 mb-6">Submit an issue related to your transport services.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            className="inline-flex items-center px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500"
          >
            Submit Complaint
          </button>
        </div>
